document.addEventListener("DOMContentLoaded", function() {
	...
	const open__certificate = document.querySelectorAll('.works__modal')[0]
	const open__biruang = document.querySelectorAll('.works__modal')[1]
	const open__pricing = document.querySelectorAll('.works__modal')[2]
	const open__design = document.querySelectorAll('.works__modal')[3]
	const open__atlantida = document.querySelectorAll('.works__modal')[4]
	const open__smitler = document.querySelectorAll('.works__modal')[5]
	...
	const modal__certificate = document.querySelector('#certificate')
	const modal__rezume = document.querySelector('#rezume')
	...
	for (let i = 0; i < close.length; i++) {
		...
		ars.onclick = function () {
			...
			modal__certificate.style.display = 'none'
		}
	}
	...
	open__certificate.onclick = function () {
			modal__certificate.style.display = 'block'
	}

	window.onclick = function (event) {
		if (event.target === modal__biruang || event.target === modal__pricing 
			|| event.target === modal__design || event.target === modal__atlantida
			|| event.target === modal__smitler || event.target === modal__certificate) {
			modal__biruang.style.display = 'none'
			modal__pricing.style.display = 'none'
			modal__design.style.display = 'none'
			modal__atlantida.style.display = 'none'
